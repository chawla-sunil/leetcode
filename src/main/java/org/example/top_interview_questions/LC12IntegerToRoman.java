...
//    Constraints:
//            1 <= num <= 3999
// This is more of a generic solution but both solutions are good use in interview
// Should tell this first
public String intToRoman(int num) {
    ...
    for (int i = roman.length - 1; i >= 0; i--) {
        while (num >= number[i]) {
            sb.append(roman[i]); // res = res + roman[i];
            num = num - number[i];
        }
    }
    ...
    // This is good and easy hack solution but only works for this question which is till 3999
    // Can be told in interview
