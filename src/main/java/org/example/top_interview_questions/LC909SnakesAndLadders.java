    ...
    public int snakesAndLadders(int[][] board) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 1; i <= 6 && curr + i <= n*n; i++) {
                int nextValue = curr + i;

                int row = (nextValue - 1) / n;
                int col = (nextValue - 1) % n;

                // since it is zigzag, this is why column is selected like this.
                int boardValue = board[n - 1 - row][(row % 2) == 0 ? col : n - 1 - col];

                int finalValue = boardValue != -1 ? boardValue : nextValue;
                if (finalValue == n*n) {
                    return minRolls[curr] + 1;
                }
                if (minRolls[finalValue] == -1) {
                    minRolls[finalValue] = minRolls[curr] + 1;
                    queue.offer(finalValue);
                }
            }
        }
